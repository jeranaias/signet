...
class TestBfsDeadlineFailClosed:
    ...
    def test_deadline_default_policy_blocks(self) -> None:
        ...
        #
        # v0.1.9.2: the v0.1.9.1 default flip to ``"audit_warn"`` was
        # reverted because it allowed depth-10-to-16 attacks on slow
        # CI hardware where the BFS budget fired before the cascade
        # unrolled — breaking R16's "N ≤ 16 always blocks" guarantee.
        # The actual fix that landed alongside this revert: raise
        # ``_BFS_WALL_BUDGET_SECONDS`` from 2.0 s to 10.0 s so the
        # deadline accommodates legitimate long base64 inputs without
        # surrendering the fail-closed default.
